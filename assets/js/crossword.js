...
function Crossword(words_in, clues_in){
    ...
    this.getLegend = function(grid){
            ...
            for (var c = 0; c < grid[r].length; c++) {
                ...
                for (var k in groups) {
                    ...
                    if (cell && cell[k] && cell[k]['is_start_of_word']) {
                        ...
                        positions[index] = position + ' ' + k;
                        groups[k].push({"position": position, "row": r, "col": c, "index": index, "clue": clues_in[index], "word": words_in[index], "length": words_in[index].length});
        ...
        for (var k in groups) {
            for (var i = 0; i < groups[k].length; i++) {
                var parsed_clue = groups[k][i]["clue"].replace(/\{(\d+)\}/g, function(match, placeholder) {
                    var index = parseInt(placeholder, 10);
                    return '<i>' + positions[index-1] + '</i>'
                });

                groups[k][i]["clue"] = parsed_clue;
            }
        }
        return groups;
    }	
    ...
    for(var i = 0; i < words_in.length; i++){
        word_elements.push(new WordElement(words_in[i].toLowerCase(), i));
    }
    ...
    toHtml : function(grid, show_answers ){       
        ...
        for(var r = 0; r < grid.length; r++){
            ...
            for(var c = 0; c < grid[r].length; c++){
                ...
                if (is_start_of_word) {
                    extra_data += ' data-position="'+label+'"';
                    html.push("<td id='crossword-" + r + "-" + c + "' " + extra_data + " data-row='" + r + "' data-col='" + c + "' class='crossword-cell start " + css_class + "'>");
                    label++;			
                } else {
                    html.push("<td id='crossword-" + r + "-" + c + "'" + extra_data + " data-row='" + r + "' data-col='" + c + "' class='crossword-cell " + css_class + "'>");					
                }
                ...
                if(show_answers) {
                    ...
                    html.push("&nbsp;");								
                   //html.push('<input id="cw-input-'+r+'-'+c+'" >');								
                }
            }
            html.push("</tr>");
        }
